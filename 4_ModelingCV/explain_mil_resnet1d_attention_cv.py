...
------------------------------------
Attention-based explainability for 1D ResNet MIL model trained with
...
Uses **top 5%** of segments per patient as "important" (by attention weight).

Usage: python explain_mil_resnet1d_attention_cv.py \
  --save_dir mil_runs \
  --run_name resnet1d_mil_cv \
  --data_root ../../ECG_data \
...
# ===============================================================
# ResNet1D + MIL (copied from training)
# ===============================================================
...
class ResNet1D(nn.Module):
    def __init__(self, in_ch=1, base=64, emb_dim=128):
        ...
        self.stem = nn.Sequential(
            nn.Conv1d(
                in_ch, base, kernel_size=7, stride=2, padding=3, bias=False
            ),
            nn.BatchNorm1d(base),
...
class ExplainConfig:
    ...
    run_name: str = "resnet1d_mil_cv"
    explain_dir: str = "explain_resnet_attention"
...
def run_explain(cfg: ExplainConfig):
    ...
    for fold, (train_idx, val_idx) in enumerate(
        skf.split(meta, meta["outcome_label"]), start=1
    ):

        print(f"\n========== ResNet Explainability: Fold {fold}/{cfg.n_splits} ==========")
...
def main():
    parser = argparse.ArgumentParser(
        description="Explain MIL ResNet1D attention across CV folds."
    )
    ...
    parser.add_argument("--run_name", type=str, default="resnet1d_mil_cv")
    parser.add_argument("--explain_dir", type=str, default="explain_resnet_attention")
