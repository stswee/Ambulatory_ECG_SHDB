...
# -*- coding: utf-8 -*-
"""
overlay_attention_ecg_resnet1d_cv.py
------------------------------------
Overlay MIL attention on raw ECG segments for ResNet1D MIL CV model.

For a given fold and patient:
    - Load attention vector from explain_mil_resnet1d_attention_cv.py
...
Usage: python overlay_attention_ecg_resnet1d_cv.py \
  --data_root ../../ECG_data \
...
def overlay_attention_with_ecg(
    ...
    if len(seg_files) != nseg:
        print(
            f"[Warning] #segment files ({len(seg_files)}) != len(attn) ({nseg}); "
            "matching to min length."
        )
    ...
    ax_attn.set_title(f"ResNet1D Fold {fold} - Patient {pid_str} - Attention")
    ...
    for i, seg_idx in enumerate(top_idx_to_plot, start=1):
        ...
        ax.set_title(
            f"Top segment {i}: index={seg_idx}, attention={attn[seg_idx]:.4f}"
        )
    ...
    if output_path is None:
        output_path = os.path.join(
            fold_dir,
            f"resnet_fold{fold}_patient_{pid_str}_overlay_top{int(top_fraction*100)}pct.png",
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Overlay MIL attention on ECG segments for ResNet1D MIL CV model."
    )
    parser.add_argument(
        "--data_root",
        type=str,
        default="../../ECG_data",
        help="Root directory of ECG data.",
    )
    parser.add_argument(
        "--segment_dirname",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.05,
        help="Fraction of highest-attention segments to consider.",
    )
    parser.add_argument(
        "--max_plots",
        type=int,
