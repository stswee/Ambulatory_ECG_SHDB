...
# -*- coding: utf-8 -*-
"""
train_mil_efficientnet1d_cv_crossattn.py
----------------------------------------
MIL with 1D EfficientNet backbone + Time→Frequency Cross-Attention Fusion.
...
class ECGMILDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for tf in time_files:
            ...
            freq_file = os.path.join(self.freq_root, pid, f"{base}_amp.npy")
            if not os.path.exists(freq_file):
                raise FileNotFoundError(f"Missing freq file: {freq_file}")

            arr_f = np.load(freq_file)
...
# ---------------------------------------------------------------
# EfficientNet1D backbone
# ---------------------------------------------------------------
class SE1D(nn.Module):
    def __init__(self, C, reduction=8):
        super().__init__()
        self.fc1 = nn.Conv1d(C, C//reduction, 1)
        self.fc2 = nn.Conv1d(C//reduction, C, 1)
...
class MBConv1D(nn.Module):
    def __init__(self, in_ch, out_ch, expand=2, stride=1, kernel=7):
        super().__init__()
        mid = in_ch * expand
        pad = kernel//2
        ...
        self.depthwise = nn.Sequential(
            nn.Conv1d(mid, mid, kernel, stride, pad, groups=mid, bias=False),
            nn.BatchNorm1d(mid),
            nn.SiLU()
        )
    ...
    def forward(self, x):
        ...
        if self.use_res: out = out + x
        return out

class EfficientNet1D(nn.Module):
    def __init__(self, in_ch=1, width=1.0, emb_dim=128):
        super().__init__()
        base = int(16 * width)
        ...
        self.blocks = nn.Sequential(
            ...
            MBConv1D(base*2, base*2),
            MBConv1D(base*2, base*4, stride=2),
            MBConv1D(base*4, base*4),
            MBConv1D(base*4, base*4)
        )
...
# ---------------------------------------------------------------
# Cross-Attention Fusion (Time → Frequency)
# ---------------------------------------------------------------
...
class MILEfficientNetCrossAttn(nn.Module):
    def __init__(self, in_ch=1, emb_dim=128):
        super().__init__()
        self.enc_t = EfficientNet1D(in_ch, emb_dim=emb_dim)
        self.enc_f = EfficientNet1D(in_ch, emb_dim=emb_dim)
    ...
    def forward(self, ts, fs):
        ...
        Hfused, alpha_mod = self.fusion(Ht, Hf)
...
def evaluate(model, loader, device):
    ...
    return dict(
        AUC=auc, ACC=accuracy_score(ys,preds),
...
class TrainConfig:
    ...
    run_name: str = "efficientnet_crossattn_cv"
    seed: int = 42
    n_splits: int = 5
...
def main():
    ...
    for fold, (tr, val) in enumerate(skf.split(meta, meta["outcome_label"]), 1):
        ...
        model = MILEfficientNetCrossAttn().to(device)
    ...
    m, s = summary.mean(numeric_only=True), summary.std(numeric_only=True)
    summary.loc["Mean"], summary.loc["Std"] = m, s
    out_csv = os.path.join(cfg.save_dir, f"{cfg.run_name}_summary.csv")
    summary.to_csv(out_csv)

    print("\n===== CROSS-VALIDATION SUMMARY =====")
    for k in ["AUC","ACC","Precision","Recall","F1"]:
        print(f"{k:>10}: {m[k]:.4f} ± {s[k]:.4f}")
    print("Saved:", out_csv)
